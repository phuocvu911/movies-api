...
func (r *GenreRepository) GetAll(limit, offset int) ([]models.Genre, int, error) {
	...
	if total == 0 {
		return nil, 0, customerrors.NotFoundf("No genres found")
	}

	if total < offset {
		return nil, total, customerrors.NotFoundf("Page is out of range")
	}

	rows, err := r.db.Query("SELECT id, name FROM genres ORDER BY id LIMIT ? OFFSET ?", limit, offset)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
...
func (r *GenreRepository) GetMoviesByGenreID(genreID int64, limit, offset int) ([]models.Movie, int, error) {
	...
	if total == 0 {
		return nil, 0, customerrors.NotFoundf("No movies found for genre ID %d", genreID)
	}
